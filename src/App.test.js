...
test("render missions data when the API call finishes", async () => {
  mockFetchShow.mockReturnValueOnce(showsData);

  render(<App />);

  screen.findByText(/Fetching data.../i);

  //   waitFor(() => screen.getAllByTestId("dropdown"));
  //   expect(screen.getAllByTestId("dropdown")).toBeInTheDocument();

  const dropDown = await screen.findAllByText(/Select a season/i);
  //   console.log(dropDown);
  //   userEvent.click(dropDown);

  //   const select1 = screen.findAllByText(/Season 1/i);
